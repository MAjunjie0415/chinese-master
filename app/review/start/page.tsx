...
export default async function ReviewStartPage({ searchParams }: PageProps) {
  ...
  let countResult, sourceResult, masteredResult;

  try {
    [countResult, sourceResult, masteredResult] = await Promise.all([
      // 查询待复习单词总数
      db
        .select({ count: count() })
        .from(userProgress)
      ...
      db
        ...
        .where(
          and(
            eq(userProgress.userId, userId),
            sql`${userProgress.masteryScore} >= 100`
          )
        ),
    ]);
  } catch (error) {
    console.error('Error fetching review start data:', error);
    // If fetching stats fails, rely on the main review page to handle the flow or show empty state
    // We can redirect to /review which now has robust error handling
    redirect('/review');
  }
